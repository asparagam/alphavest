    ...
    <header className="h-16 bg-dark-surface/80 border-b border-dark-border/80 sticky top-0 z-20 backdrop-blur-xl px-4 lg:px-8 flex items-center justify-between gap-4">
      <div className="flex items-center gap-4">
        ...
        <form onSubmit={handleSearchSubmit} className="hidden sm:flex items-center relative w-64 lg:w-80">
          ...
          <input
            type="text"
            placeholder="Search ticker (e.g. NVDA, AAPL, BTC)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-dark-card/90 border border-dark-border text-xs text-slate-100 placeholder-slate-400 pl-10 pr-8 py-2 rounded-xl focus:border-brand-500 focus:outline-none"
          />
          <kbd className="hidden lg:inline-block absolute right-3 text-[10px] font-mono text-slate-400 bg-slate-800 px-1.5 py-0.5 rounded">
            ⌘K
          </kbd>
        </form>
      </div>
